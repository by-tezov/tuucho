...
class StyleCustomLabelDimensionRectifierMatcher : RectifierMatcherProtocol {
    override fun accept(
        path: JsonElementPath,
        element: JsonElement
    ): Boolean {
        if (!path.lastSegmentIs(CustomLabelSchema.Style.Key.fontSize)) return false
        val parent = element.find(path.parent())
        return parent.isTypeOf(TypeSchema.Value.style) &&
                parent.isSubsetOf(CustomLabelSchema.Component.Value.subset)
    }
}
