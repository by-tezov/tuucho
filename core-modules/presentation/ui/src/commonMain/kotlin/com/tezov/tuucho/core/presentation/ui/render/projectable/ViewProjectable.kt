...
class ViewProjectable : ProjectableProtocol {
    ...
    fun <T : ProjectionProtocol> newProjection(
        ...
        (when (klass) {
            ViewsProjection.Static::class -> ViewsProjection.Static(key, screen, path)
            ViewsProjection.Mutable::class -> ViewsProjection.Mutable(key, screen, path)
            ViewProjection.Static::class -> ViewProjection.Static(key, screen, path)
            ViewProjection.Mutable::class -> ViewProjection.Mutable(key, screen, path)
...
inline fun <reified T : ProjectionProtocol> ViewProjectable.projection(
    key: String,
    screen: Screen,
    path: JsonElementPath,
