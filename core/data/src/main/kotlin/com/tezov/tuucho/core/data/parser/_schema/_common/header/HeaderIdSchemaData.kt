package com.tezov.tuucho.core.data.parser._schema._common.header
...
interface HeaderIdSchemaData {
    ...
    companion object {
        ...
        fun String.idGetGroupOrNull(): String? {
            val parts = removePrefix(ID_REF_INDICATOR)
                .split(ID_PATH_SEPARATOR, limit = 2)
            return if (parts.size == 2) parts[0] else null
        }

        val Map<String, JsonElement>.idExist get() = this[Name.id] != null
        val Map<String, JsonElement>.idIsObject get() = this[Name.id] is JsonObject
        val Map<String, JsonElement>.idIsPrimitive get() = this[Name.id] is JsonPrimitive

        val Map<String, JsonElement>.idObject get() = this[Name.id]!!.jsonObject
        val Map<String, JsonElement>.idRawOrNull get() = this[Name.id]
        val Map<String, JsonElement>.id get() = this[Name.id].string
        val Map<String, JsonElement>.idOrNull get() = this[Name.id].stringOrNull
        ...
        fun MutableMap<String, JsonElement>.idPutObject(value: String, source: String?) {
            put(Name.id, JsonObject(mutableMapOf<String, JsonElement>().apply {
                idPut(value, source)
            }))
        }

        fun MutableMap<String, JsonElement>.idPut(value: String?, source: String?) {
            value?.let { put(Name.Id.value, JsonPrimitive(it)) }
            source?.let { put(Name.Id.source, JsonPrimitive(it)) }
        }
